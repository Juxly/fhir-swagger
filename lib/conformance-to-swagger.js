...
Converter.convert = function (baseURL, conf) {
  ...
  swagger.definitions = {};
  swagger.paths = {};
  swagger.info = { description: conf.description, title: conf.name, version: '2.0' };
  var url = URL.parse(baseURL || conf.url);
  ...
  resources.forEach(function (res) {
    ...
    swagger.definitions[res.type] = getSchema(res);
    ...
    if (interactions.indexOf('history-instance') !== -1) {
      var histByID = swagger.paths['/' + res.type + '/{id}/_history'] = { get: getDefaultOp(res) };
      histByID = histByID.get;
      histByID.parameters = [
    ...
    if (interactions.indexOf('history-type') !== -1) {
      var histOp = swagger.paths['/' + res.type + '/_history'] = { get: getDefaultOp(res) };
      histOp = histOp.get;
      histOp.parameters = [
        { name: 'id', in: 'path', type: 'string' },
        { name: '_count', in: 'query', type: 'string' },
